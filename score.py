...
def get_convex_hull(contour):
    return cv2.convex_Hull(contour)
...
def get_contour_area_by_bounding_box_area(contour):
    return (cv2.contourArea(contour) / cv2.contourArea(get_bounding_rect()))


def get_contour_perim_by_hull_perim(contour):
    return (cv2.arcLength(contour, True) / cv2.arcLength(get_convex_hull(contour), True))


def get_contour_perim_by_bounding_box_perim(contour):
    return (cv2.arcLength(contour, True) / cv2.arcLength(get_bounding_rect(contour), True))
...
# Default mutable argumentssould be harmless here
def draw_point(point, image, radius=5, color=(0, 0, 255)):
...
def order_points(points):
    """order point contour clockwise-ly."""
...
def get_outmost_points(contours):
    all_points = np.cancatenate(contours)
    return get_bounding_rect(all_points)



def perspective_transform(img,points):
    """transform image so that points are the new cornes"""
    source=np.array(points,dtype="float32")
    dest=np.array([TRANSF_SIZE,TRANSF_SIZE],[0,TRANSF_SIZE],[0,0],[TRANSF_SIZE,0],dtype="float32")
    img_dest=img.copy
...
def sheet_coord_to_transf_coord(x,y):
    return list(map(lambda n:int(np.round(n)),(TRANSF_SIZE * x/744.055,TRANSF_SIZE * (1 - y/1052.362))))



def get_question_patch(transf,q_number):
    #top left
    tl=sheet_coord_to_transf_coord(200,- 80 * (q_number - 1))

    #Bottom right
    br=sheet_coord_to_transf_coord(650,800 - 80 * (q_number - 1))

    return transf[tl[1]:br[1],tl[0],br[0]]

def get_question_patch(transf):
    for i in range(1,11):
        yield get_question_patch(transf,i)


def get_alternative_patch(question_patch,index):
    for i in range(5):
...
def draw_marked_alternative(question_patch,index):
    cx,cy=sheet_coord_to_transf_coord(50*(2*index + 0.5),50/2)
...
def get_marked_alternative(alternative_patches):
    ...
    # Simple Heuristic
    if sorted_mean[0]/sorted_mean[1]> .7:
        return None
    return np.argmin(means)

def get_letters(alt_index):
    return ["A","B","C","D","E"][alt_index] if alt_index is not None else "N/A"


def get_answer(source_file):
    ...
    answers=[]
    for i,q_patch in enumerate(get_question_patch(transf)):
        alt_index=get_marked_alternative(get_marked_alternative(q_patch))

    answers.append(get_letters(alt_index))
...
def main():
    ...
    answers,im=get_answer(args.input)

    for i,answers in enumerate(answers):
        print("Q{}: {}".format(i+1,answers))

    if args.ouput:
        cv2.imweite(args.output,im)
        print("Wrote image to {}".format(args.output))

    if args.show:
        cv2.imshow('trans',im)
        print("close image window and hit ^C to quite")
        while True:
            cv2.waitKey()
